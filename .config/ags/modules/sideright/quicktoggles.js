...
export const ToggleIconBluetooth = (props = {}) =>
  Widget.Button({
    ...
    onClicked: () => {
      ...
      else exec("rfkill unblock bluetooth");
    },
    onSecondaryClickRelease: () => {
...
export const PowerProfilesModule = () =>
  Widget.Button({
    className: "txt-small sidebar-iconbutton",
    tooltipText: PowerProfiles.bind('active_profile'),
    onClicked: () => {
      const status = PowerProfiles.active_profile;
      if(status == "balanced") {
        PowerProfiles.active_profile = "performance"
      } else if (status == "performance") {
        PowerProfiles.active_profile = "power-saver"
      } else if (status == "power-saver") {
        PowerProfiles.active_profile = "balanced"
      }
    },
    child: PowerProfilesIndicator(),
    setup: (self) => {
      setupCursorHover(self);
  ...
  return Widget.Button({
    ...
    onClicked: (self) => {
      ...
      if (self.attribute.enabled)
        Utils.execAsync("hyprsunset --temperature 5000").catch(print);
      else
        Utils.execAsync("pkill hyprsunset")
          .then(() => {
            // disable the button until fully terminated to avoid race
            self.sensitive = false;
            const source = setInterval(() => {
              Utils.execAsync("pkill -0 hyprsunset").catch(() => {
                self.sensitive = true;
                source.destroy();
              });
            }, 500);
...
export const ModuleCloudflareWarp = async (props = {}) => {
  ...
  return Widget.Button({
    ...
    onClicked: (self) => {
      ...
      self.toggleClassName("sidebar-button-active", self.attribute.enabled);
      if (self.attribute.enabled)
        Utils.execAsync("warp-cli connect").catch(print);
...
export const ModuleRawInput = async (props = {}) => {
  try {
    ...
    return Widget.Button({
      ...
      onClicked: (button) => {
        Hyprland.messageAsync("j/getoption input:accel_profile").then(
          (output) => {
            ...
            if (value != "[[EMPTY]]" && value != "") {
              execAsync([
                "bash",
                "-c",
                `hyprctl keyword input:accel_profile '[[EMPTY]]'`,
              ]).catch(print);
              button.toggleClassName("sidebar-button-active", false);
            } else {
              Hyprland.messageAsync(`j/keyword input:accel_profile flat`).catch(
                print
              );
              button.toggleClassName("sidebar-button-active", true);
            }
