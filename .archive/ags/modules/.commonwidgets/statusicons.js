...
export const PowerProfilesIndicator = () =>
	Widget.Stack({
		...
		children: {
			balanced: Widget.Label({ className: 'txt-norm icon-material', label: 'balance' }),
			performance: Widget.Label({ className: 'txt-norm icon-material', label: 'speed' }),
			'power-saver': Widget.Label({ className: 'txt-norm icon-material', label: 'battery_saver' }),
		},
		setup: (self) =>
			self.hook(PowerProfiles, (stack) => {
				stack.shown = String(PowerProfiles.active_profile);
			}),
	});

export const SpeakerMuteIndicator = () =>
	Widget.Revealer({
...
// export const BluetoothIndicator = () => Widget.Label({ className: 'txt-norm icon-material', label: 'bluetooth' })
...
const HyprlandXkbKeyboardLayout = async ({ useFlag } = {}) => {
	try {
		...
		const updateCurrentKeyboards = () => {
			var initLangs = [];
			JSON.parse(Utils.exec('hyprctl -j devices')).keyboards.forEach((keyboard) => {
...
const createKeyboardLayoutInstances = async () => {
	...
	const instances = await Promise.all(Array.from({ length: monitorsCount }, () => OptionalKeyboardLayout()));
