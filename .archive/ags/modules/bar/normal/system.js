...
export const Utilities = () => Box({
    hpack: 'center',
    className: 'spacing-h-4',
    children: [
        UtilButton({
            name: getString('OCR from screenshot'), icon: 'screenshot_region', onClicked: () => {
                // OCR with tesseract
                Utils.execAsync([`bash`, `-c`, `grim -g "$(slurp $SLURP_ARGS)" "tmp.png" && tesseract -l eng+deu+jpn "tmp.png" - | wl-copy && rm "tmp.png"`])
                    .catch(print)
            }
        }),
        UtilButton({
...
export const BatteryModule = () => Stack({
